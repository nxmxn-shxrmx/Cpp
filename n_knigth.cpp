...
bool isvalid(int i ,int j,vector<vector<int>>v)
{
   ...
   int d[8][2]={  {-1,-2},  {-2,-1},  {-2,1},  {-1,2} , {1,2 } , {2,1},   {2,-1} , {1,-2}};
    for(auto c:d)
    {
        if(i+c[0]>=0 && i+c[0]<v.size() && j+c[1]>=0 && j+c[1]<v.size())
        {
            if(v[i+c[0]][j+c[1]])
            return 0;
        }
...
map<string,bool>m;
void f(int i,int j,vector<vector<int>>&v,string s,int n)
{
    ...
    if(isvalid(i,j,v))
    {
        ...
        f(i,l,v,s+"{"+to_string(i)+"-"+to_string(j)+"} ",n-1);

          for(int l = i+1;l<=v.size();++l)
         {
            for(int r = 0;r<=v.size();++r)
                f(l,r,v,s+"{"+to_string(i)+"-"+to_string(j)+"} ",n-1);
        }
        v[i][j]=0;
    }
    v[i][j]=0;
